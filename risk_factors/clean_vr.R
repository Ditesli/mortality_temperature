...
clean_vr = function(df, paths, varnames, iso3, icdVersion) {
  ...
  print("Merging in ICD map and applying age/sex restrictions...")
  big <- merge(df, icdmap, by = 'value', all.x = TRUE)
  print(paste0(round(100*mean(is.na(big$cause_id)), 1), "% (n=", sum(is.na(big$cause_id)), ") of rows failed to merge to ICD map on 1st attempt"))

  # where icd codes failed to link to gbd cause, add a decimal to position 4 and re-merge to icd map
  if (icdVersion==10) {
    big[(cause_id==743 | is.na(cause_id)==TRUE) & nchar(value)>=4 & grepl("\\.", value)==F, 
        value := paste0(substr(value, 1, 3), ".", substr(value, 4, nchar(value)))]
  } else if (icdVersion==9) {
    big[(cause_id==743 | is.na(cause_id)==TRUE) & nchar(value)>=4 & grepl("\\.", value)==F & substr(value, 1, 1)!="E",
        value := paste0(substr(value, 1, 3), ".", substr(value, 4, nchar(value)))]
  }

  big <- big[, 'cause_id' := NULL]
  big <- merge(big, icdmap, by = 'value', all.x = TRUE)    
  print(paste0(round(100*mean(is.na(big$cause_id)),1), "% (n=", sum(is.na(big$cause_id)), ") of rows failed to merge to ICD map after adding missing decimals"))
  ...
  big <- big[, 'cause_id' := NULL]
  big <- merge(big, icdmap, by = 'value', all.x = TRUE)
  print(paste0(round(100*mean(is.na(big$cause_id)),1), "% (n=", sum(is.na(big$cause_id)), ") of rows failed to merge to ICD map after removing decimal precision"))
